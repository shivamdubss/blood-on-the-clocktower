...
export const abilityHandler: AbilityHandler = (context, input) => {
  const data = input as { player1Id?: string; player2Id?: string; answer?: boolean } | undefined;
  if (!data?.player1Id || !data?.player2Id || typeof data.answer !== 'boolean') {
    return { success: false, message: 'Must provide player1Id, player2Id, and answer (boolean)' };
  }
